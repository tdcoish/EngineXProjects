...
void InputManager::HandleSingleEvent(SDL_Event* pE) {
	...
	if (pE->type == SDL_KEYDOWN)
		...
		HandleMouseButtonPressed(pE->button.button);
	else if (pE->type == SDL_MOUSEBUTTONUP)
		HandleMouseButtonReleased(pE->button.button);
...
void InputManager::HandleMouseButtonPressed(Uint8 btn) {
	...
	case SDL_BUTTON_LEFT: 
		...
		if (!(kStore^LMB)) {
			msLeftClickedThisFrame = true;
		}
		else {
			msLeftClickedThisFrame = false;
		}
	...
	case SDL_BUTTON_RIGHT:
		// This really should need a ! in front of it.
		if ((kStore^RMB)) {
			msRightClickedThisFrame = true;
		}
		else {
			msRightClickedThisFrame = false;
		}
